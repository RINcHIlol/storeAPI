...
//	// Создаем временные файлы для всех картинок и собираем cid для каждого
...
//	}
//
//	// Формируем HTML с товарами: картинка слева, текст справа
//	productsHtml := ""
//	for i, product := range products {
...
//
//	//// Тут настройка и отправка письма через SMTP (зависит от твоей конфигурации)
...
func MailToCustomer(products []models.ProductCount, order models.Order, finalPrice float64) (*mail.Message, []*os.File, error) {
	// Создаем временные файлы для картинок
	cids := make([]string, len(products))
	tmpFiles := make([]*os.File, len(products))
	...
	buyer := mail.NewMessage()
	buyer.SetHeader("From", "galimatron229@gmail.com")
	buyer.SetHeader("To", "workemailsvarka123@gmail.com") // или order.CustomerEmail
	buyer.SetHeader("Subject", "Shopping in Svarka_Shop")
